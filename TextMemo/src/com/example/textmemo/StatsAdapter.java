...
public class StatsAdapter extends BaseAdapter {
	...
	public StatsAdapter( Context context, ArrayList<MemoInfo> data ) {
	...
	public View getView(int position, View convertview, ViewGroup parent) {
		...
		if( v == null ) {
			...
			// 가공하여 그 텍스트를 뷰에 설정한다.
			info.writeDate.setText( listData.get( position ).memo );

			// 레이아웃 객체로부터 메모 내용을 저장하기 위한 텍스트 뷰 객체 정보를 가져와  
			// 메모 내용을 설정하고
			info.memo = (TextView)v.findViewById( R.id.memostring );
			info.memo.setText( listData.get( position ).subject );
